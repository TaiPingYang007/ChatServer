...
bool Redis::publish(int channel, std::string message) {
  ...
  if (reply == nullptr) {
    std::cerr << "publish command faild!\n";
    return false;
  }
...
bool Redis::subscribe(int channel) {
  ...
  // redisBufferWrite可以循环发送缓冲区，指导缓冲区数据发送完毕（done被置成1）
  int done = 0;
  while (!done) {
...
void Redis::observer_channel_message() {
  ...
  if (_notify_message_handler == nullptr) {
    // 如果还没注册就收到消息了，先打印个错误日志防身
    std::cerr << "Fatal Error: Notify Handler not initialized!" << std::endl;
    return;
  }
  ...
  std::cerr << ">>>>>>>>>> observer_channel_messagee quit <<<<<<<<<<"
            << std::endl;
}
